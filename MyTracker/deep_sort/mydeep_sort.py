...
from .sort.mydetection import Detection
#from .sort.tracker import Tracker
from .sort.mytracker import Tracker
...
class MyDeepSort(object):
    ...
    def update(self, bbox_xywh, confidences, cls_ids, ori_img):
        ...
        boxes = np.array([d.tlwh for d in detections])#array([189., 646., 109.,  72.]), same to bbox_tlwh
        scores = np.array([d.confidence for d in detections])
        indices = non_max_suppression(boxes, self.nms_max_overlap, scores)
        detections = [detections[i] for i in indices]#no effect, all pass
